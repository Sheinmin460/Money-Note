...
const AdminDashboard: React.FC = () => {
  ...
  const openAdjustmentModal = (method: string) => {
  ...
  const handleAdjustBalance = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedMethod || !initialAmount) return;
    ...
    <div className="p-4 sm:p-6 bg-slate-50 min-h-screen">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage your finances and wallet corrections.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link to="/">
            <button className="px-4 py-2 text-sm font-semibold bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 transition-all shadow-sm">
              Exit
            </button>
          </Link>
        </div>
          ...
          {loading ? (
            ...
            balances.map((balance) => (
              <div key={balance.payment_method} className="relative group rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 hover:shadow-md transition-all">
                ...
                <button
                  onClick={() => openAdjustmentModal(balance.payment_method)}
                  className="absolute top-4 right-4 p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all opacity-0 group-hover:opacity-100 shadow-sm border border-transparent hover:border-emerald-100"
      ...
      <Modal
        open={isModalOpen}
        title={`Adjust ${selectedMethod} Balance`}
      ...
      >
        <form onSubmit={handleAdjustBalance} className="space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Adjustment Amount</label>
            <div className="relative">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className={`px-8 py-3 text-sm font-bold text-white rounded-xl shadow-xl transition-all active:scale-95 disabled:opacity-50 ${adjustmentType === 'income' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700'
                }`}
            >
